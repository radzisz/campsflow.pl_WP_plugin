...
final class EventMetaWidget extends Widget_Base
{
    ...
    private function registerContentSection(): void
    {
        ...
        $this->add_control('show_photos',        ['label' => __('Zdjęcia', 'campsflow'),           'type' => $switcher, 'default' => '',    'label_on' => $yes, 'label_off' => $no]);
        $this->add_control('show_location',      ['label' => __('Lokalizacja', 'campsflow'),        'type' => $switcher, 'default' => 'yes', 'label_on' => $yes, 'label_off' => $no]);
        $this->add_control('show_tags',          ['label' => __('Tagi', 'campsflow'),               'type' => $switcher, 'default' => 'yes', 'label_on' => $yes, 'label_off' => $no]);
        $this->add_control('show_description',   ['label' => __('Opis ogólny', 'campsflow'),        'type' => $switcher, 'default' => 'yes', 'label_on' => $yes, 'label_off' => $no]);
        $this->add_control('show_program',       ['label' => __('Program', 'campsflow'),            'type' => $switcher, 'default' => '',    'label_on' => $yes, 'label_off' => $no]);
        $this->add_control('show_price_include', ['label' => __('Co zawiera cena', 'campsflow'),    'type' => $switcher, 'default' => '',    'label_on' => $yes, 'label_off' => $no]);
        $this->add_control('show_documents',     ['label' => __('Dokumenty', 'campsflow'),          'type' => $switcher, 'default' => '',    'label_on' => $yes, 'label_off' => $no]);
        $this->add_control('show_terms',         ['label' => __('Warunki ogólne', 'campsflow'),     'type' => $switcher, 'default' => '',    'label_on' => $yes, 'label_off' => $no]);
        $this->add_control('show_instructions',  ['label' => __('Inf. praktyczne', 'campsflow'),    'type' => $switcher, 'default' => '',    'label_on' => $yes, 'label_off' => $no]);
        $this->add_control('show_contact',       ['label' => __('Kontakt', 'campsflow'),            'type' => $switcher, 'default' => '',    'label_on' => $yes, 'label_off' => $no]);
    ...
    protected function render(): void
    {
        ...
        if ($s['show_photos'] === 'yes')        $this->echoPhotos($postId);
        if ($s['show_location'] === 'yes')       $this->echoLocation($loc);
        if ($s['show_tags'] === 'yes')           $this->echoTags($postId);
        if ($s['show_description'] === 'yes')    $this->echoDescription($postId);
        if ($s['show_program'] === 'yes')        $this->echoProgram($desc);
        if ($s['show_price_include'] === 'yes')  $this->echoPriceInclude($desc);
        if ($s['show_documents'] === 'yes')      $this->echoDocuments($postId);
        if ($s['show_terms'] === 'yes')          $this->echoTerms($postId);
        if ($s['show_instructions'] === 'yes')   $this->echoInstructions($postId);
        if ($s['show_contact'] === 'yes')        $this->echoContact($postId);
        echo '</div>';
    }
    ...
    private function echoLocation(?array $loc): void
    {
        ...
        if ($titleParts) echo '<strong>' . esc_html(implode(' — ', $titleParts)) . '</strong>';
        $addrParts = array_filter([$street, $city]);
        if ($addrParts) echo '<span>' . esc_html(implode(', ', $addrParts)) . '</span>';
        ...
        if ($webpage) echo '<a href="' . esc_url($webpage) . '" target="_blank" rel="noopener">' . esc_html($webpage) . '</a>';
        echo '</div>';
    }
    ...
    private function echoTags(int $postId): void
    {
        ...
        if ((! $tags || is_wp_error($tags)) && (! $ages || is_wp_error($ages))) return;
        echo '<div class="cf-event-body__tags">';
        if ($tags && ! is_wp_error($tags)) {
            foreach ($tags as $tag) echo '<span class="cf-tag">' . esc_html($tag->name) . '</span>';
        }
        if ($ages && ! is_wp_error($ages)) {
            foreach ($ages as $age) echo '<span class="cf-tag cf-tag--age">' . esc_html($age->name) . '</span>';
        }
        echo '</div>';
    }
    ...
    private function echoContact(int $postId): void
    {
        ...
        if ($name)  echo '<div class="cf-contact__name">' . esc_html($name) . '</div>';
        if ($email) echo '<a href="mailto:' . esc_attr($email) . '">' . esc_html($email) . '</a>';
        if ($phone) echo '<a href="tel:' . esc_attr($phone) . '">' . esc_html($phone) . '</a>';
        echo '</div>';
    }
