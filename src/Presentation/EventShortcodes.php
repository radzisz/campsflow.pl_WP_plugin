...
final class EventShortcodes
{
    ...
    public function renderMeta(array|string $atts): string
    {
        ...
        if (in_array('photos', $show, true))        $this->echoPhotos($postId);
        if (in_array('location', $show, true))       $this->echoLocation($loc);
        if (in_array('tags', $show, true))           $this->echoTags($postId);
        if (in_array('description', $show, true))    $this->echoDescription($postId);
        if (in_array('program', $show, true))        $this->echoProgram($desc);
        if (in_array('price_include', $show, true))  $this->echoPriceInclude($desc);
        if (in_array('documents', $show, true))      $this->echoDocuments($postId);
        if (in_array('terms', $show, true))          $this->echoTerms($postId);
        if (in_array('instructions', $show, true))   $this->echoInstructions($postId);
        if (in_array('contact', $show, true))        $this->echoContact($postId);
        echo '</div>';
